    ...
    public:
        ...
        virtual double eval_f(const Vec &_x) override {
            ...
            for(int i=0; i<springs_.size(); ++i) {
                coeff[0] = ks_[i];
                coeff[1] = ls_[i];
                // springs_[i] is an Edge (pair of indices) eg. (x_a, x_b)
                // get global positions (_x) to local (xe_)
        ...
        virtual void eval_gradient(const Vec &_x, Vec &_g) override {
            ...
            for(int i=0; i<springs_.size(); ++i) {
                ...
                // assemble the local gradient (ge_) to the global gradient vector (_g)
        ...
        virtual void eval_hessian(const Vec &_x, SMat& _h) override {
           ...
           for(int i = 0; i < springs_.size(); ++i) {
                ...
                // global indices of spring i
                std::array<int, 4> global_indices = {
                    2 * springs_[i].first,
                    2 * springs_[i].first + 1,
                    2 * springs_[i].second,
                    2 * springs_[i].second + 1
                };
                
                // get local coordinates
                for(int j = 0; j < 4; ++j) {
                    xe_[j] = _x[global_indices[j]];
                }
                
                func_.eval_hessian(xe_, coeff, he_);

                // assemble the local hessian (he_) to the global hessian (_h)
                for(int i = 0; i < 4; ++i) {
                    for(int j = 0; j < 4; ++j) {
                        triplets.emplace_back(global_indices[i], global_indices[j], he_(i, j));
                    }
                }
            }
            //------------------------------------------------------//
