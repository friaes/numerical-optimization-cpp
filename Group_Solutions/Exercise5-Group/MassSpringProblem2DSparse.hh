 ...
 * only concerns matrices. */
    ...
    public:
        ...
        virtual double eval_f(const Vec &_x) override {
            ...
            for (size_t j = 0; j < attached_node_indices_.size(); ++j) {
                cs_xe_.resize(2);
                const int v = attached_node_indices_[j];
                cs_xe_[0] = _x[2*v];
                cs_xe_[1] = _x[2*v + 1];
                
                Vec coeff2(3);
                coeff2[0] = weights_[j];
                coeff2[1] = desired_points_[2*j];
                coeff2[2] = desired_points_[2*j + 1];

                energy += cse_.eval_f(cs_xe_, coeff2);
            }
        ...
        virtual void eval_gradient(const Vec &_x, Vec &_g) override {
            ...
            //use cs_ge_ to store the gradient of the attached node index
                ...
                for (size_t j = 0; j < attached_node_indices_.size(); ++j) {
                    ...
                    cse_.eval_gradient(cs_xe_, coeff2, cs_ge_);
                    _g[2*v    ] += cs_ge_[0];
                    _g[2*v + 1] += cs_ge_[1];
                }
        ...
        virtual void eval_hessian(const Vec &_x, SMat& _h) override {
            ...
            for (size_t j = 0; j < attached_node_indices_.size(); ++j) {
                ...
                cse_.eval_hessian(cs_xe_, coeff2, cs_he_);
