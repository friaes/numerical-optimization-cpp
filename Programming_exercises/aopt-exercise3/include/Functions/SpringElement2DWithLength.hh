 ...
 * l_ab which represents the length of the spring at rest. */
    ...
    public:
        ...
        inline virtual void eval_hessian(const Vec &_x, const Vec &_coeffs, Mat &_H) override {
            ...
            _H(0, 0) = k * (4*x*x + 2*y*y - 2*l_sq);
            _H(0, 1) = k * (2*x*y);
            _H(0, 2) = -k * (4*x*x + 2*y*y - 2*l_sq);
            _H(0, 3) = -k * (2*x*y);
            _H(1, 0) = k * (2*x*y);
            _H(1, 1) = k * (2*x*x + 4*y*y - 2*l_sq);
            _H(1, 2) = -k * (2*x*y);
            _H(1, 3) = -k * (4*y*y + 2*x*x - 2*l_sq);
            _H(2, 0) = -k * (4*x*x + 2*y*y - 2*l_sq);
            _H(2, 1) = -k * (2*x*y);
            _H(2, 2) = k * (4*x*x + 2*y*y - 2*l_sq);
            _H(2, 3) = k * (2*x*y);
            _H(3, 0) = -k * (2*x*y);
            _H(3, 1) = -k * (4*y*y + 2*x*x - 2*l_sq);
            _H(3, 2) = k * (2*x*y);
            _H(3, 3) = k * (2*x*x + 4*y*y - 2*l_sq);
            //------------------------------------------------------//
        }
    };
